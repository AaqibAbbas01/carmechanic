...
export function CompanySettings() {
  ...
  return (
    <Panel icon={<Building2 />} title="Company & WhatsApp">
      ...
      <div className="mt-4 flex flex-col items-center gap-3 rounded-lg border border-zinc-800 bg-black p-3 sm:flex-row sm:items-start">
        {store.company.logoUrl ? (
          ...
          <div className="flex h-24 w-24 shrink-0 items-center justify-center rounded-md bg-[#d3121c] text-xl font-black text-white">{store.company.logoText}</div>
        )}
        <div className="min-w-0 w-full text-center sm:text-left">
          ...
          <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:flex-wrap">
            ...
            {store.company.logoUrl ? (
              <button className="w-full rounded-md px-3 py-2 text-sm font-semibold text-[#ff525a] sm:w-auto" onClick={() => updateCompany({ logoUrl: "" })}>Remove</button>
            ) : null}
          </div>
